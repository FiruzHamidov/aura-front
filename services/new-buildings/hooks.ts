...
export const useDevelopers = (params = defaultParams) =>
    useQuery({
        ...
        queryFn: async () => {
            const { data } = await axios.get<Paginated<Developer>>('/developers', { params });
...
export const useConstructionStages = (params = defaultParams) =>
    useQuery({
        ...
        queryFn: async () => {
            const { data } = await axios.get<Paginated<ConstructionStage>>('/construction-stages', { params });
            return data;
        },
        staleTime: 5 * 60 * 1000,
    });
...
export const useMaterials = (params = defaultParams) =>
    useQuery({
        ...
        queryFn: async () => {
            const { data } = await axios.get<Paginated<Material>>('/materials', { params });
            return data;
        },
        staleTime: 5 * 60 * 1000,
    });
...
export const useFeatures = (params = defaultParams) =>
    useQuery({
        ...
        queryFn: async () => {
            const { data } = await axios.get<Paginated<Feature>>('/features', { params });
            return data;
        },
        staleTime: 5 * 60 * 1000,
    });
...
export const useLocations = (params = defaultParams) =>
    useQuery({
        ...
        queryFn: async () => {
            const { data } = await axios.get<Paginated<LocationOption>>('/locations', { params });
            return data;
        },
        staleTime: 5 * 60 * 1000,
    });
...
// ===== Новостройки =====
export interface NewBuildingsQuery {
    page?: number;
    per_page?: number;
    developer_id?: number;
    stage_id?: number;
...
export const useNewBuildings = (params: NewBuildingsQuery = {}) =>
    useQuery<Paginated<NewBuilding>>({
        queryKey: ['new-buildings', params],
        queryFn: async () => {
            const { data } = await axios.get<Paginated<NewBuilding>>('/new-buildings', { params });
            return data;
        },
...
export const useUpdateNewBuilding = (id: number) => {
    ...
    return useMutation({
        mutationFn: async (payload: NewBuildingPayload) => {
            const { data } = await axios.put<NewBuilding>(`/new-buildings/${id}`, payload);
            return data;
        },
...
export const useDetachFeature = () =>
    useMutation({
        mutationFn: async (vars: { newBuildingId: number; featureId: number }) => {
            const { newBuildingId, featureId } = vars;
            await axios.delete(`/new-buildings/${newBuildingId}/features/${featureId}`);
            return true;
        },
    });