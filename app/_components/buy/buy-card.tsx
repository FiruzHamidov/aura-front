...
const BuyCard: FC<BuyCardProps> = ({listing, user, isLarge = false}) => {
    ...
    const userRole = user?.role?.slug === 'admin' ? 'admin' : user?.role?.slug === 'agent' ? 'agent' : null;
    ...
    //   listing.description || listing.landmark || 'Описание отсутствует';
    ...
    const displayFloorInfo =
        listing.floor && listing.total_floors
            ? `${listing.floor}/${listing.total_floors} этаж`
            : 'Этаж не указан';
    ...
    const displayAgent = listing.creator
        ? {
            ...
            role: 'риелтор',
            photo: listing.creator.photo
                ? `${STORAGE_URL}/${listing.creator.photo}`
                : '',
        }
        : null;
        ...
        <div
          ...
          <span className="font-bold text-[#020617]">
            {formattedPrice} {displayCurrency}
                ...
                {/*  {displayDescription.substring(0, 55)}...*/}
                {/*</p>*/}
            ...
            {isModalOpen && userRole && (
                <ModerationModal
                    ...
                    onUpdated={(updated) => {
                        // можно обновить listing локально, если нужно
