...
const ALL_ITEMS: Record<string, MenuItem> = {
    reports: {href: '/profile/reports', label: 'Отчёты', icon: <ReportsIcon className="w-5 h-5"/>},
    profile: {href: '/profile', label: 'Профиль', icon: <UserIcon className="w-5 h-5"/>},
    favorites: {href: '/profile/favorites', label: 'Избранное', icon: <HeartIcon className="w-5 h-5"/>},
    myList: {href: '/profile/my-listings', label: 'Мои объявления', icon: <School className="w-5 h-5"/>},
    allList: {href: '/profile/all-listings', label: 'Все объявления', icon: <School className="w-5 h-5"/>},
    booking: {href: '/profile/my-booking', label: 'Мои показы', icon: <CalendarIcon className="w-5 h-5"/>},
    addPost: {href: '/profile/add-post', label: 'Добавить объявление', icon: <PlusIcon className="w-5 h-5"/>},
    users: {href: '/admin/users', label: 'Пользователи', icon: <UsersIcon className="w-5 h-5"/>},
    buildings: {href: '/admin/new-buildings', label: 'Новостройки', icon: <Building className="w-5 h-5"/>},
};

const roleMenus: Record<RoleSlug, Array<keyof typeof ALL_ITEMS>> = {
    admin: ['reports', 'profile', 'allList', 'buildings', 'booking', 'users', 'favorites', 'addPost'],
...
export const Sidebar = () => {
    ...
    return (
        <>
            ...
            <div className="mx-auto max-w-[1520px] px-4 mt-5 px-7 hidden md:block sticky top-2 z-30 ">
