...
export const Sidebar = () => {
  ...
  const newPath = '/profile' + (pathname.split('/').slice(2).join('/') ? '/' + pathname.split('/').slice(2).join('/') : '');
  ...
  return (
      <>
        ...
        <button className="md:hidden absolute top-20 left-4 z-50" onClick={() => setIsOpen(true)}>
        ...
        >
          ...
          <nav className="flex flex-col gap-3">
            {[
              { href: '/profile', icon: <UserIcon className="w-6 h-6" />, label: 'Профиль' },
              { href: '/profile/favorites', icon: <HeartIcon className="w-6 h-6" />, label: 'Избранное' },
              // { href: '/profile/wallet', icon: <WalletIcon className="w-6 h-6" />, label: 'Кошелек' },
              { href: '/profile/my-listings', icon: <ListingIcon className="w-6 h-6" />, label: 'Мои объявления' },
              { href: '/profile/my-booking', icon: <CalendarIcon className="w-6 h-6" />, label: 'Мои показы' },
              { href: '/profile/add-post', icon: <PlusIcon className="w-6 h-6" />, label: 'Добавить объявление' },
            ].map(({ href, icon, label }) => (
                <Link
                    key={href}
                    href={href}
                    className={`flex items-center gap-3 p-3 rounded-3xl font-medium ${
                        activeLink === href ? 'bg-[#0036A5] text-white' : 'text-gray-700 hover:bg-gray-100'
                    }`}
                    onClick={() => {
                      setActiveLink(href);
                      setIsOpen(false);
