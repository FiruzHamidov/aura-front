...
// ---- Локальный тип с опциональными связями (без any)
...
export default function NewBuildingsIndexPage() {
    ...
    // Явно приводим ответ к Paginated<NewBuilding>
                    ...
                    {(items as NBWithRelations[]).map((nb) => (
                        <tr key={nb.id} className="border-t">
                            ...
                            <td className="px-4 py-3">
                                {nb.completion_at ? new Date(nb.completion_at).toLocaleDateString() : '-'}
                            </td>
