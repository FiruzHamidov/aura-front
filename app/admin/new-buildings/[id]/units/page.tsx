...
export default function BuildingUnitsPage() {
  ...
  const pagination = (unitsResponseRaw ??
    null) as Paginated<BuildingUnit> | null;
                  ...
                  {unitsList.map((unit: BuildingUnit) => {
                    return (
                      <tr key={unit.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {unit.price_per_sqm
                            ? Number(unit.price_per_sqm).toLocaleString()
                            : '—'}{' '}
                          {unit.currency ?? ''}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {unit.total_price
                            ? Number(unit.total_price).toLocaleString()
                            : '—'}{' '}
                          {unit.currency ?? ''}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <span
                            className={`px-2 py-1 rounded-full text-xs ${
                              unit.moderation_status === 'available'
                                ? 'bg-green-100 text-green-800'
                                : unit.moderation_status === 'sold'
                          ...
                          >
                            {unit.moderation_status === 'available'
                              ...
                              : unit.moderation_status === 'sold'
                              ? 'Продана'
                              : unit.moderation_status === 'reserved'
                              ? 'Забронирована'
                              : 'Неизвестно'}
                          </span>
                        </td>
            ...
            <div className="flex items-center gap-2">
              ...
              {Array.from(
                ...
                <button
                  ...
                  onClick={() => setPage(p)}
                  className={`px-3 py-1 border rounded ${
                    p === (pagination?.current_page ?? 1) ? 'bg-gray-200' : ''
                  }`}
                >
                  {p}
                </button>
              ...
              <button
                onClick={() =>
                  setPage((p) => Math.min(pagination?.last_page ?? p, p + 1))
                }
                disabled={Boolean(
                  pagination &&
