...
import { useNewBuilding, useUpdateNewBuilding } from '@/services/new-buildings/hooks';
...
import type {NewBuildingPayload, NewBuilding, LocationOption} from '@/services/new-buildings/types';
import { toast } from 'react-toastify';

const STEPS = ['Основная информация', 'Детали и локация'];

// Узкий тип для ошибки API, чтобы не использовать any
type ApiError = {
    response?: {
        data?: {
            message?: string;
...
export default function NewBuildingEditPage() {
    ...
    const locationOptions: LocationOption[] = (locations?.data ?? []) as LocationOption[];
    useEffect(() => {
        if (!initial) return;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload: NewBuildingPayload = {
                ...
                construction_stage_id: form.construction_stage_id ? Number(form.construction_stage_id) : null,
    ...
    return (
        <FormLayout title="Редактировать новостройку" description="Обновите данные ЖК">
            <ProgressIndicator currentStep={step} totalSteps={2} steps={STEPS} className="mb-8" />
