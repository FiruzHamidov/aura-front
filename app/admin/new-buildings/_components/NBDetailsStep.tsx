...
export default function NBDetailsStep({
    ...
    // react-yandex-maps ожидает ref вида MutableRefObject<Map | undefined>
    const mapRef = useRef<ymaps.Map | undefined>(undefined);
    const ymapsRef = useRef<typeof ymaps | null>(null);

    // Хелпер, чтобы безопасно дергать onChange без any
    const fireChange = (name: string, value: string) => {
        const evt = { target: { name, value } } as unknown as ChangeEvent<HTMLInputElement>;
        onChange(evt);
    };
                ...
                <div className="h=[380px] w-full h-[380px]">
                    <YMaps query={{ lang: 'ru_RU', apikey: 'dbdc2ae1-bcbd-4f76-ab38-94ca88cf2a6f' }}>
                        <Map
                            ...
                            instanceRef={mapRef}
                            modules={['geocode']}
                            onLoad={(y) => (ymapsRef.current = y)}
                        >
                            {coordinates && (
                                <Placemark
                                    geometry={coordinates}
                                    options={{ preset: 'islands#blueHomeIcon', draggable: true }}
                                    properties={{ iconCaption: addressCaption || 'Определение адреса...' }}
                                />
                            )}
                        </Map>
                    </YMaps>
